...
long long Ordinal1(int num, long long min, long long max) {
//	printf("%ld\n", sizeof(long long));
	srand(num);
...
//		printf("wrong\n");
	...
	while( part != NULL ) {
		...
		*/
		switch(j) {
			case 1:
				test ^= Ordinal1(0x4, 0x9E3779B9, 0xffffffff);
//				printf("case 1: %llx\n", test);
				break;
			case 2:
				test ^= Ordinal1(0x8, 0xefcdab89, 0xffffffff);
//				printf("case 2: %llx\n", test);
				break;
			case 3:
				test ^= Ordinal1(0xf, 0x61C88647, 0xffffffff);
//				printf("case 3: %llx\n", test);
				break;
		}
...
float Ordinal5(char *str) {
	...
	char* hex = (char*)malloc(32*sizeof(char));
	char *no_use;
	long long res_cmp[4] = {0xb07fa95f, 0x5ef510b6, 0x413f02f9};
...
//		计算每一部分的长度
		...
		switch(j) {
			case 0:
				test ^= Ordinal4(0x11223344);
				break;
			case 1:
				test ^= Ordinal4(0xedfac873);
				break;
			case 2:
				test ^= Ordinal4(0xaecfed81);
				break;
		}
		printf("test = %lx, %lx\n", test, res_cmp[j]);
		// if(test == res_cmp[j])
...
//		first 最后等于 3
	...
	// printf("first = %d", first+(int)flag);
	return ((int)flag + (int)first);
}
...
// 测试一下在dllmain里添加检测启动器函数, 这么些dllmain是可以的
