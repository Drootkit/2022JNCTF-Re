...
void creatStr(int choice)
{
	...
	switch (choice)
	{
	case 0x11:			//wrong flag这是main里的判断不对
...
unsigned char* GetProces()
{
	...
	char* ida = (char*)malloc(20);
	char* dbg = (char*)malloc(20);

	strcpy(ida, "ida64.exe");
	strcpy(dbg, "x64dbg.exe");


	if (hProcess == INVALID_HANDLE_VALUE)
	{
		printf("CreateToolhelp32Snapshot() call Failed!\n");
		exit(0);
	}
	...
	while (bMore)
	{
		if (!strcmp(ida, currentProcess.szExeFile) || !strcmp(dbg, currentProcess.szExeFile))
		{
			flag = 0x62;
			break;
		}

		if (!strcmp("notepad.exe", currentProcess.szExeFile))
		{
			flag = 0x23;
		}

		if (!strcmp("Calculator.exe", currentProcess.szExeFile))
		{
			flag += 0x20;
			if (flag == 0x43)
				break;
...
void ror7rol(unsigned char* str, int n, int w)
{
	...
	case 0x74:
		...
		break;
	default:
		printf("plz input the right choice\n");
		break;
	}
}
...
int main()
{
	...
	unsigned char cmp[] = { 0x88, 0x2a, 0xaa, 0x2a, 0x4b, 0x8a,
							...
							0x6c, 0x4a, 0x29, 0xee, 0x69, 0xd,
							0x7, 0xac, 0x29, 0x2d, 0x6, 0xea,
							0xa8, 0xaa, 0x6e, 0xee, 0x48, 0x48,
							0x86, 0xe6, 0xe9, 0x28, 0xee, 0xf,
							0x49, 0x8a, 0x6d, 0x2e, 0xe8, 0x4f,
							0x6d, 0xed, 0xa9, 0x4d, 0x6e, 0xae,
							0x9, 0xee, 0x86, 0xae, 0xa, 0x4f, 0xee,
							0x46, 0xe8, 0xb, 0x7, 0xee, 0x4b, 0xf,
							0x29, 0x2f, 0x0 };
